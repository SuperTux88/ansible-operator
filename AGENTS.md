...
src/v1beta1/
  ...
  controllers/playbookplancontroller/
    ...
    node_recreation.rs               drops a host's recorded application when its Node's uid differs from `appliedNodeUid` — a rebuilt machine inherits the name, never the claim. Level-triggered on purpose: a deletion the operator was down for leaves no event to react to
