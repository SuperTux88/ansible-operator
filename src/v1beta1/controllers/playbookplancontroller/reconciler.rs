...
async fn reconcile(
    ...
    let mut recovered_run_still_in_flight = false;
    ...
    if let Some(recovered) = recovered {
        match recovered {
            ...
            RecoveredRun::Finished {
                ...
                recovered_run_still_in_flight = others_in_flight;
            }
        }
    }
        ...
        && let Some(mirror) = resource_status.active_run.clone()
    {
        ...
        match progress {
            ...
            ActiveRunProgress::Finished {
                ...
                recovered_run_still_in_flight = false;
            }
        }
    }
    ...
    if let Some(unlaunched) = unlaunched_run {
        ...
        if recovered_run_still_in_flight {
...
///
/// `others_in_flight` says whether the plan still has a *different* attempt behind this result — a
/// terminal record is drained ahead of anything live, so a tick can apply one run's outcome while
/// another is genuinely running. The slot is then left alone: `lastTriggeredRun` is the sole dedupe
/// key for "one run per schedule window", and stamping the finished run's window over a live
/// attempt's would describe a run that is already over. The live attempt records its own slot when
/// it completes.
///
...
fn sync_desired_hash_after_finished_run(
    ...
    if finished.execution_hash == *desired_hash {
        if !others_in_flight {
            record_triggered_slot(status, finished.mirror.triggered_slot);
        }
...
async fn recover_active_run(
    ...
    if let Some(play) = unacknowledged.first() {
        ...
        return Ok(Some(RecoveredRun::Finished {
            ...
            others_in_flight: !live.is_empty(),
        }));
    }
...
enum RecoveredRun {
    ...
    Finished {
        ...
        /// Whether another attempt of this plan is still in flight behind the drained result. A
        /// terminal result is handed over ahead of anything live, so the plan is *not* finished
        /// when this is set, and the tick must not classify it as such.
        others_in_flight: bool,
    },
}
...
spec:
  ...
  template:
    ...
    /// between a schedule window and a second run inside it, so it must keep describing the attempt
    /// the plan is actually holding — not the one that has already finished. The live attempt
    /// records its own slot when it completes.
    #[test]
