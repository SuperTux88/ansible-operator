...
- *Residual:* a `plays` writer can still abort a run in its **own** namespace by making a record stop
  ...
  availability problem rather than an escalation one. Grant `update` on `plays` to **nobody**: a
  record's spec is written once at creation and never edited, so not even the operator holds that
  verb (see the RBAC table in §8 — it has `get,list,create,delete` on `plays`, and `update` only on
  `plays/status`).
- *Severity:* Low.
