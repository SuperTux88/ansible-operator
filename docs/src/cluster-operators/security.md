...
`plays/status`, which should be granted to the operator alone. `update` on `plays` itself should be
granted to **nobody**: a record's spec is written once at creation and never edited, so the operator
does not hold that verb either — the chart grants it only `get`, `list`, `create` and `delete`.
